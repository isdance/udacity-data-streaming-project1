...
class Station(Producer):
    ...
    def run(self, train, direction, prev_station_id, prev_direction):
        ...
        print(f"train arrived this station", value)
        try:
            self.producer.produce(
                topic=self.topic_name, key={"timestamp": self.time_millis()}, value=value
            )
        except Exception as e:
            logger.error(f"station.run", e)
